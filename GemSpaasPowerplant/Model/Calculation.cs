    ...
    public class Calculation
    {
        ...
        private IEnumerable<PowerLoad> GeneratePlan()
        {
            ...
            while (i < myOrderedPowerPlants.Count)
            {
                ...
                delta = load - myOrderedPowerPlants.MatchedLoad();
                deltaPMin = load - myOrderedPowerPlants.PMinTotal();

                if (delta > 0) //still load to compensate, select max extra power from this pp
                {
                    ...
                    powerplant.p = (int)Math.Min(delta, powerplant.pmax * correctionIfWind);
                    if (powerplant.pmin > delta) // not enough power to be generated by this pp, remove some from previous
                    {
                        powerplant.p = powerplant.pmin;
                        missingPMin =  powerplant.pmin - delta;
                        i--;
                        continue;
                    }
                    i++;
                ...
                if (delta == 0) //load is already fulfilled
                {
                    i++;
                    continue;
                }

                if (delta < 0) //need to drop the power of  plant
                {
