...
from powerplant.models import PowerPlantLoad, PowerPlant, PowerPlantType, \
    FuelsCosts, ProductionPlanRequest, PlantLoadResponse
...
def test_power_plan_load_should_raise_value_error():
    plant = PowerPlant(
        name="gasfiredbig1", type="gasfired", efficiency=0.53, pmin=100,
        pmax=460
    )
...
def test_should_compute_cost():
    ...
    efficiency = 1
    plant = PowerPlant(
        name="gasfiredbig1", type=PowerPlantType.GAS_FIRED.value,
        efficiency=efficiency, pmin=100, pmax=460
    )
    ...
    efficiency = 1
    plant = PowerPlant(
        name="turbojet1", type=PowerPlantType.TURBO_JET.value,
        efficiency=efficiency, pmin=100, pmax=460
    )
    ...
    efficiency = 1
    plant = PowerPlant(
        name="wind1", type=PowerPlantType.WIND_TURBINE.value,
        efficiency=efficiency, pmin=100, pmax=460
    )
...
def test_should_optimize_load_with_low_consumption():
    ...
    gas_plant = PowerPlant(
        name="gas1", type=PowerPlantType.GAS_FIRED.value,
        efficiency=efficiency, pmin=100, pmax=460
    )
    efficiency = 1
    wind_plant = PowerPlant(
        name="wind1", type=PowerPlantType.WIND_TURBINE.value,
        efficiency=efficiency, pmin=0, pmax=30
    )

    request = ProductionPlanRequest(load=10, fuels=fuels_costs,
                                    powerplants=[gas_plant, wind_plant])
...
def test_should_optimize_combine_load():
    ...
    request = ProductionPlanRequest(load=160, fuels=fuels_costs,
                                    powerplants=[gas_plant, wind_plant])
...
def test_should_optimize_load_taking_into_account_pmin():
    ...
    request = ProductionPlanRequest(load=110, fuels=fuels_costs,
                                    powerplants=[gas_plant, wind_plant])
